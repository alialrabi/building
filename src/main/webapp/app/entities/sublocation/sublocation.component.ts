...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.sublocationService.search({
    ...
    transition() {
        this.router.navigate(['/sublocation'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Sublocation) {
        return item.id;
    }
    registerChangeInSublocations() {
        this.eventSubscriber = this.eventManager.subscribe('sublocationListModification', (response) => this.loadAll());
    }
